            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $data -> email }}</td>
                        <td>{{ $data -> name }}</td>
                        <td><img src="{{ asset('../img/account/'.$data -> profile_pict) }}" alt="Avatar" class="img-fluid img-thumbnail" style="max-height: 80px" /></td>
                        <td>
                            <a class="btn btn-sm btn-primary" href="/admin-area/akun/edit/{{ Crypt::encrypt($data -> id) }}/{{ 0 }}">
                                <span class="align-middle">Edit</span>
                            </a>
                            <button onclick="if (confirm('Hapus akun {{ $data -> name }}')) { location.replace('/admin-area/akun/delete/{{ Crypt::encrypt($data -> id) }}/0') }" class="btn btn-danger btn-sm">
